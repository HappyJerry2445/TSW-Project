...
public class ProductDetailsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			NotificationUtil.sendNotification(req, "ID prodotto non valido.", "error");
			resp.sendRedirect(req.getContextPath() + "/");
			return;
		}
		...
		try{
			...
			if(product == null){
				NotificationUtil.sendNotification(req, "Prodotto non trovato.", "error");
				resp.sendRedirect(req.getContextPath() + "/");
				return;
			}

			ProductImageDTO image = productImageDAO.getFirstByProductId(productId);
