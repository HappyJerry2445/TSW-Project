...
public class ProductDAO implements GenericDAO<Product, Integer> {
    private static final List<String> ALLOWED_ORDER_COLUMNS = Arrays.asList(
            "ProductId", "SKU", "ProductName", "BasePrice", "CurrentPrice", "StockQuantity", "CategoryId", "ProductType", "CreatedAt", "LastUpdated", "IsActive"
    );
    ...
    public void save(Product product) throws SQLException {
        if(product == null || product.getSku() == null || product.getSku().trim().isEmpty() || product.getProductName() == null || product.getProductName().trim().isEmpty() ||
                product.getBasePrice() < 0 || product.getCurrentPrice() < 0 || product.getStockQuantity() < 0 || product.getCategoryId() < 0 ||
                product.getProductType() == null || product.getCreatedAt() == null) {
            throw new IllegalArgumentException("Product, SKU, ProductName, BasePrice, CurrentPrice, StockQuantity, CategoryId, ProductType, CreatedAt cannot be null, empty or < 0.");
        }

        String sql;
        if(product.getProductId() == 0) {
            sql = "INSERT INTO Product (ProductId, SKU, ProductName, BasePrice, CurrentPrice, StockQuantity, CategoryId, ProductType, CreatedAt, LastUpdated, IsActive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            try(Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                ps.setString(8, product.getProductType().name());
                ps.setTimestamp(9, (product.getCreatedAt() != null) ? Timestamp.valueOf(product.getCreatedAt()) : Timestamp.valueOf(LocalDateTime.now()));
                ps.setTimestamp(10, (product.getLastUpdated() != null) ? Timestamp.valueOf(product.getLastUpdated()) : null);
            ...
            sql = "UPDATE Product SET SKU = ?, ProductName = ?, BasePrice = ?, CurrentPrice = ?, StockQuantity = ?, CategoryId = ?, ProductType = ?, CreatedAt = ?, LastUpdated = ?, IsActive = ? WHERE ProductId = ?";
            try(Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ps.setString(8, product.getProductType().name());
                ps.setTimestamp(8,(product.getCreatedAt() != null) ? Timestamp.valueOf(product.getCreatedAt()) : null);
